...
- `desc` 도 싣는다(2026-08-26) — 톱3 카드의 한 줄 설명에 쓴다(§7.5). 100행 스냅샷이라 크기 영향은 무시할 수준.
...
- 웹 렌더는 §7.5 로 옮겼다(톱3 카드 / 컴팩트 줄). `url` 이 있으면 이름 자체가 링크 —
  **아웃바운드라 `?ref=opencodewar`** (§18).

---

### 7.4 순위 변동 (`delta`) — 전일 대비

랭킹 항목에 `delta`(양수 = 그만큼 올라섬, 0 = 유지, `null` = 직전 창에 없던 신규)를 싣는다.

- 직전 창(`prevDayFilter`): **all → 어제까지 누적**, **daily → 어제 하루**. 웹이 실제로 보여주는
  두 보드만 계산하고 weekly·weekend·monthly 는 건너뛴다(쓸 화면이 없다).
- 구현: 같은 정렬 규칙으로 `user_id → 순위` 맵을 한 번 더 뽑아(`computePrevRankMap`) 지금 순위와 뺀다.
  스냅샷 리빌드당 쿼리 4개 추가(2보드 × 2지표) — 5분에 한 번이라 무시할 수준.
- **구역(국가·도시) 보드에는 안 붙인다** — 실시간 D1 계산이라 조합마다 쿼리가 배로 는다.
  웹은 `'delta' in row` 로 판단해 없는 보드에선 아무것도 그리지 않는다.
- 스냅샷 스키마가 바뀌므로 `SNAPSHOT_KEY` 를 **v5** 로 올렸다.

### 7.5 보드 렌더 — 톱3 카드 + 한 줄 (2026-08-26)

표(`#`/`coder`/`shipping`/`prompts`/`chars`/`activity`/`cc` 7칸)를 버렸다. **정보가 없어서가 아니라
전부 같은 크기라서** 1위 줄과 10위 줄이 스캔했을 때 구분이 안 됐다. 시안: `mockups/leaderboard-hook.html`
(A/B/C 3안 비교), `mockups/leaderboard-live.html`(적용 화면).

- **톱3는 카드**(`.lb-pod`): 파비콘 32px · 닉네임 16px · 계급 배지 · 국기 · 변동 → 그 아래
  **main shipping 이름(13.5px 볼드) + MAIN 배지 + 도메인**, 그 아래 한 줄 설명.
  1위 자리가 곧 제일 좋은 광고 지면이 된다(§12 명함첩 방향).
- **4위부터는 한 줄**(`.lb-cmp`): 순위 · 닉네임 · 계급 · 국기 · 변동 · shipping(아이콘+이름) · 숫자.
- **막대는 칸에서 꺼내 행 배경 게이지로**. `activity` 칸이 사라져 가로가 넓어지고 대비는 세졌다.
  스케일은 **√** — 1위 5,145 대 4위 70 같은 판에서 선형이면 하위권이 1% 실선이 되어 안 보인다.
- **숫자 계층**: 주지표(현재 metric)만 크게, 보조는 11px 회색.
- **계급 배지**(§16)는 전 기간 누적 기준이라 `all` 보드에서만 그린다(daily 에 붙이면 그날 값으로
  계급을 매기는 것처럼 읽힌다).
- **내 위치 바**(`.mebar`, sticky): 검색하지 않아도 "나는 몇 등, 앞사람까지 몇 개" 가 항상 보인다.
  닉네임(`ocw_me`, localStorage)이 보드에 있을 때만.
- 클래스 이름은 `.lb-*` / `.lbship` 로 접두어를 붙였다 — 유저 상세의 `.ship`(shipping 카드)과
  이름이 겹치면 그쪽 박스 스타일을 그대로 뒤집어쓴다(실제로 한 번 밟았다).
...
2~15자를 통과하지 못하면 저장이 막힌다. 가입 직후 `?setup=1` 폼도 같은 규칙을 쓴다.
...
## 20. 프로젝트별 프롬프트 추적 — "그날 그날 어디에 시간을 썼나" (2026-07-31)

### 20.1 목표

유저 상세 페이지에서 프롬프트가 **어느 프로젝트에서 나왔는지**를 일별/시간별로 보여준다.
프로필의 shipping(등록 프로젝트, 최대 5개)이 홍보 카드에서 "실제로 시간을 쏟고 있는 것"의
증거로 승격된다 — 터미널 SNS·명함첩 방향과 시너지.

### 20.2 원칙 — 명시적 링크(opt-in)만 전송한다

`cwd` 디렉토리명을 그대로 보내면 회사·비공개 레포 이름이 서버로 유출된다.
§9(프라이버시 = 설치율) 원칙에 따라 **유저가 직접 링크한 폴더의, 유저가 직접 정한 라벨만** 전송한다.

- `/ocw project link <이름>` — **현재 폴더**를 그 이름에 연결. 매핑은 로컬
  `~/.open-code-war/config.json` 의 `projectDirs: { "<절대경로>": "<라벨>" }` 에만 저장(경로는 서버로 안 감).
- `/ocw project unlink` — 현재 폴더 연결 해제. `link` 인자 없이 실행하면 현재 폴더의 연결 상태 표시.
- 링크 안 된 폴더의 프롬프트는 `project` 없이 전송(= 서버에 NULL) — 기존과 동일, 유출 0.
- 라벨은 shipping 이름과 달라도 허용(≤40자). 단 공개 표기는 §20.4 규칙을 따른다.

### 20.3 수집·저장

- 훅 stdin(JSON)의 `cwd` 를 `projectDirs` 에 **최장 접두사 매칭**(경로 경계 기준)
  → 매칭되면 payload 에 `project: <라벨>` 추가. 하위 폴더에서 쳐도 상위 링크에 귀속.
- `events` 에 `project TEXT` 컬럼 추가(0012). NULL = 미지정.
- **`daily_stats` 는 건드리지 않는다** — 상세 페이지는 이미 원시 `events` 재집계(§상세 TZ 모델)이므로
  일별 분해가 공짜로 나온다. 쓰기 행 증가 0(§13 write-diet 유지). 리더보드도 무관.
- 서버 검증: 문자열, trim 후 1~40자, 제어문자 불가 — 아니면 NULL 취급.

### 20.4 공개 규칙 — shipping 에 있는 이름만, 나머지는 "기타"

- 공개 응답(`/user`·`/user/hours`)의 일별/시간별 `projects` 분해에서
  **shipping(users.projects)에 있는 이름과 일치(대소문자 무시)하는 라벨만 실명**으로 낸다.
  캐노니컬 표기는 shipping 쪽 이름을 따른다.
- 그 외(비매칭 라벨 + 미지정 NULL)는 전부 키 `""` 하나로 합산 — 웹이 "기타"로 표기.
- **본인 예외 없음**(2026-08-26 변경): 로그인해 자기 상세를 봐도 비매칭 라벨은 "기타"다.
  자동 집계(§20.7)를 켜면 디렉토리 이름이 차트 범례에 그대로 깔리는데, 화면 공유·스크린샷에
  회사 레포 이름이 새는 경로였다. shipping 에 등록한 이름 = 보여줄 작정인 이름, 그 외는 전부 "기타".
  (세션 판별(`isOwner`)은 잠수함 익명화와 응답의 shipping 목록에만 남는다.)
- shipping 에서 프로젝트를 지우면 그 라벨은 즉시 "기타"로 강등된다(과거 이벤트 포함) — 별도 마이그레이션 불필요.

### 20.5 웹 표기

- 상세 차트는 **최근 30일 일별 하나**로 고정(2026-08-26). 시간별 "day" 뷰와 30d/day 토글은 제거했다 —
  하루짜리 시간 분포는 남에게 볼 이유가 적고, 필터가 늘수록 첫 화면이 무거워진다.
  백엔드 `/user/hours` 는 공개 API 로 남는다(에이전트용, §24).
- 스택 기준 토글: **에이전트 ↔ 프로젝트**. 프로젝트 데이터가 하나도 없으면 토글·범례를 숨긴다.
- 프로젝트 색: shipping 순서대로 고정 팔레트 5색 + 기타(회색). 범례·툴팁에 라벨 표기.

### 20.6 남은 것 / 비범위

- ⬜ codex·opencode·pi 어댑터의 stdin 에는 `cwd` 가 없을 수 있음 — 없으면 미지정(NULL)으로 동작.
  어댑터별 cwd 전달은 추후.
- ⬜ OG 카드에 프로젝트 분포 반영은 비범위.
- ⬜ 리더보드 행에 프로젝트별 수치는 비범위(대표 프로젝트 칩은 기존 §7.3 유지).

### 20.7 확장: 디렉토리 자동 집계 · 잠수함 모드 · 단독 차트 (2026-07-31)

시안: `mockups/project-stealth.html` (본인/공개 뷰 토글로 아래 규칙 전부 시연).

**디렉토리 자동 집계 — `/ocw project auto on|off` (옵트인, 기본 off)**

- 링크 안 된 폴더의 프롬프트도 **폴더 이름만** 라벨로 전송한다(`config.projectAuto`).
  §20.2 원칙 유지: 전체 경로는 어떤 경우에도 서버로 가지 않는다.
- 그룹핑 키는 **git 저장소 루트**의 폴더 이름(`autoProjectLabel`) — 하위 폴더에서 쳐도
  한 프로젝트로 묶인다. git 밖이면 cwd 폴더 이름, 40자 초과는 자름.
- 명시 링크(`projectDirs`)가 항상 우선. 서버·공개 규칙은 그대로다 —
  자동 라벨은 비 shipping 라벨이므로 본인이 보든 남이 보든 전부 "기타"로 합산된다(§20.4).

**잠수함 모드 — `/ocw project submarine <번호|이름>` (토글, `projects[].sub`)**

- shipping 에 등록하되 **공개 지면에는 이름을 숨긴다**. 공개 응답(`publicProjects`)에서
  `{name: "secret #n", secret: true}` 로 익명화 — desc·url·main 제거, **개수와 활동량만** 노출.
- `n` 은 목록 순서상 잠수함 중 몇 번째(1-base). 여러 척이면 secret #1·#2… 로 구분된다.
- 상세 집계 키도 동일 규칙(`shipDisplayMap`): 잠수함 라벨 이벤트는 공개엔 "secret #n",
  본인에겐 실명. 차트 색은 프로젝트 목록 순서 그대로라 본인/공개 뷰의 색이 일치한다.
- 리더보드 대표 칩(§7.3, `pickMainProject`)은 잠수함을 후보에서 제외 — main 이어도 안 낸다.
- 웹 표기: 공개 = 점선 카드 "🤿 secret #n" + 고정 티저 문구, 링크·설명 없음.
  본인 = 실명 + 🤿 잠수함 배지 + "공개 페이지에는 secret #n" 힌트. 편집 폼에도 🤿 체크박스
  (저장 시 sub 보존 — 없으면 웹 저장이 CLI 설정을 지운다).

**shipping 한도 5 → 10** (`MAX_PROJECTS`, 서버·CLI·웹 3곳 동일). 차트 고정 팔레트도
5색 → 10색(`PJ_COLORS`, 앞 5색은 기존 유지 — 배포된 화면과 색 호환).

**shipping 카드 클릭 → 단독 차트** (웹 전용, `uPjSel`)

- 카드를 클릭하면 30일·하루 차트가 **그 프로젝트만, 자체 최대값 기준으로** 다시 그려진다
  (스택에 묻힌 작은 프로젝트의 활동 패턴이 보이게). 스탯 타일 4칸도 그 프로젝트 값으로 전환.
- 차트 위 표시줄(이름 + "이 프로젝트만 표시 중" + ✕ 해제), 범례는 선택 외 항목 흐림.
- 다시 클릭·✕·에이전트 스택 전환·다른 유저 이동 시 해제. 퍼센트(점유율) 표기는 넣지 않는다(의도).
- 공개 뷰의 secret 카드도 클릭 가능 — 비밀 프로젝트에 쏟는 활동 패턴까지는 보인다(티저).

배포 노트: DB 마이그레이션 없음(`users.projects` JSON 안에 `sub` 만 추가).
npm 패키지(open-code-war)는 track.mjs·config.mjs 변경 포함 — 수동 재발행 + 버전 bump 필요.

---

## 21. 리포지셔닝 — 간판을 "전쟁"에서 "기록"으로 (2026-08-06)

### 21.1 문제

첫 화면 간판이 "누가 제일 많이 입력했나를 겨루는 리더보드 게임"이었는데, 경쟁은 설치 동기로 가장 약한 축이다.
리더보드는 중위권 경쟁자들만의 게임이고(§19.10에서 확인: 1위는 브리핑조차 거의 안 뜬다), 신규 유저는 1위와의
거리가 아득해 감정적으로 붙을 데가 없다. 반면 실제 가치(§19 브리핑, §20 프로젝트 추적, §16 계급, §17 스트릭)는
간판에 안 걸려 있었다.

### 21.2 결정 — 메시지 위계 3겹

1. **거울** (설치할 이유) — "에이전트가 코드를 쓰는 시대, 내 노력은 어디에 남나". 커밋 잔디가 못 담는 매일을
   기록. 혼자여도 첫날부터 유용. → 랜딩 히어로 · README 첫 줄 · og:description 등 첫인상 전부.
2. **명함** (계속 쓸 이유 + 퍼지는 이유) — 계급·스트릭·프로필·공유 카드. "에이전트 시대의 잔디밭" 포지셔닝. → 프로필 · OG 카드 · 브리핑.
3. **전쟁** (소수가 열광할 이유) — 리더보드·국가전·퍼레이드. 간판에서 내리되 제품 안에서는 그대로. 이름이
   Open Code War인 이유는 여기 남는다. → 보드 탭 이하, 히어로 아래.

리브랜딩이 아니다: 이름·전쟁 세계관·픽셀 병정·퍼레이드·리더보드 전부 유지. "기록"이 문이고 "전쟁"이 방이다.

### 21.3 1차 반영 (이 커밋)

- **웹 히어로** — 전쟁터 6인 교전 애니메이션 → **잔디밭 + 타건 병정**: 잔디 히트맵(30×4, --grass-0..4 토큰)이
  땅이 되고, 그 위에 기존 typer 스프라이트 병정 + chartflag 피크 깃발. 병정이 "지금도 기록 중"을 연기한다.
- **카피** — slogan/tagline(en·ko), head 메타(title·description·og·twitter·JSON-LD) 전부 기록 프레임으로.
- **설치 한 줄 상주** — 히어로에 마켓플레이스 명령 + 복사 버튼(.hero-install). 랜딩의 일은 구경이 아니라 설치.
- **README(ko·en)** — 첫 줄 교체 + "🪞 설치하면 보이는 것" 섹션 신설(브리핑·계급·스트릭·프로젝트·명함·보드).

### 21.4 남은 것

- 잔디를 실데이터로: 비로그인은 표시용 결정적 패턴(연출, 구 전쟁터와 같은 지위) — 로그인 유저에겐 본인 90일
  히트맵 + 병정 군복색=주력 에이전트 + 깃발=최고 기록일. 90일 일별 API 필요.
- 히어로 상태줄에 내 스트릭·내 계급 칩(로그인 시).
- og.png 메인 이미지도 기록 프레임으로 리뉴얼(교체 시 ?v=3 — 3곳 동시, §OG Cache Busting).

---

## 22. 교전 (Battle) — 최대 10명 개인전 (2026-08-06 설계)

### 22.1 왜 — §21 사다리의 4번(무리 단위 도입)

- 글로벌 보드는 모르는 사람과의 경쟁이라 감정이 안 실린다. 경쟁이 뜨거워지는 단위는 "아는 사람들"이다.
- **초대 링크가 곧 성장 루프** — 미설치자가 링크를 열면 "설치하고 참전" 랜딩. 설치의 단위를 개인 → 무리로 바꾼다.
- §19.10 공백(1위는 브리핑에 알릴 게 없다)을 메운다 — 교전 중엔 1위에게도 "2위가 12 차이로 추격 중"이라는 뉴스가 항상 있다.
- 결과 카드가 §21 사다리 3번(자랑거리)의 첫 구현이 된다 — 주간 Wrapped 는 이 렌더러의 변형으로 나중에.

### 22.2 규칙 (2026-08-06 확정)

- **개인전만, 최대 10명.** 팀전은 v1 비범위. 10명 = 브리핑 한 줄에 담기고 전원이 서로를 아는 밀도.
- **기간: 방장이 생성 시 지정, 최소 24시간 ~ 최대 7일.** 생성 즉시 시작, `ends_at = created_at + duration`.
  - 하한 24h: 시간대 공정성 — 전원이 자기 작업 시간대를 한 바퀴 돈다.
  - 상한 7d: 경쟁 긴장의 수명 + 리매치 리듬.
- **집계 창은 전원 동일** — 점수는 항상 `starts_at ~ ends_at` 사이 events. 참가 시점은 보드 등재 여부만 결정한다.
  늦게 조인해도 창 시작 이후 프롬프트가 소급 집계 → 늦참 불이익 없음, 구현도 단순.
- 지표: `prompts` 기본, 방장이 `chars` 선택 가능 (제안).
- 상금 없음 (§1 비목표 유지). 어뷰징: 아는 사이 전제 + 기존 /track rate-limit.

### 22.3 플로우 (제안)

- 진입점은 플러그인(§14.2 원칙): `/ocw battle new <이름> <기간>` (예: `3d`, `48h`) → 초대 코드 + 링크 반환.
- 참가: `/ocw battle join <코드>` 또는 웹 링크 `/battle/<코드>`.
- 진행 중: 웹 `/battle/<코드>` 페이지에서 순위 (기존 5분 스냅샷 리듬), 세션 브리핑이 중계석.
- 종료: cron 이 `status='ended'` 처리 + 결과 스냅샷 KV → **결과 카드** (전원 순위 + 픽셀 병정 시상대).
- 미설치자가 초대 링크를 열면: 교전 정보 + 코드 프리필된 설치 안내 (성장 루프의 핵심 화면).

### 22.4 데이터 (제안)

- `battles`: id, code(초대 코드, unique), name, owner_user_id, metric, created_at, ends_at, status
- `battle_members`: battle_id, user_id, joined_at (복합 PK)
- 집계는 기존 `events` 재사용 (멤버 ≤10 × 창 ≤7일 — 규모 작아 실시간 계산 부담 없음).

### 22.5 브리핑 통합 (제안)

§19.5 "우선순위 1개만" 원칙 유지, 교전 항목을 우선순위에 삽입:
스트릭 위기 > **교전 뉴스(순위 변동·종료 임박·격차)** > 계급 임박 > 글로벌 경쟁자.
동시 참가 교전이 여럿이면 종료가 가장 임박한 것 하나만.
...
## 25. 참전 기록 (Live activity) — 유일하게 움직이는 지면 (2026-08-26)

리더보드를 실시간으로 흔들지 **않는다**. 보드는 5분 스냅샷이고, 화면 숫자를 낙관적으로 올려두면
새로고침할 때 숫자가 뒤로 가는 일이 생긴다. **보드 = 5분마다 정합이 맞는 판**,
**참전 기록 = 60초마다 갱신되는 지면**으로 역할을 갈랐다. 시안: `mockups/leaderboard-live.html`.

### 25.1 API — `GET /activity`

- `events` 는 **유저 × 에이전트 × 1분** 버킷이다. 프롬프트 1건 = `events` 1행이라 그대로 흘리면
  같은 사람이 같은 초에 여러 줄로 뜬다. 1분 묶음이면 한 줄이 "그 1분에 N개 쳤다"가 되어
  60초 폴링과 결이 맞는다. 항목: `{ at, nickname, registered, public_id, agent, country, prompts, chars }`.
  `chars` 는 그 버킷의 `SUM(events.chars)` — 웹은 `+3 · 1,204자` 로 건수 옆에 흐리게 붙인다.
  `+1` 만 있으면 "1글자" 로 읽힐 수 있고, 규칙 01(문자수도 센다)이 움직이는 지면에서는 보이지 않았다(2026-08-27).
- **엣지 캐시 60초**(`caches.default`) — 방문자가 몇 명이든 D1 은 분당 1회. 조회 범위는 최근 24시간
  (인덱스 스캔 상한), 최대 50건.
- 내용은 원래 수집하지 않으므로(§9) 실을 것도 없다. `user_id` 는 비밀키라 담지 않는다
  (라우팅은 닉네임/`public_id`).

### 25.2 웹

- `setInterval(1s)` 카운트다운 + 60초마다 폴링. `document.hidden` 이면 건너뛴다 —
  안 보는 탭을 위해 D1 을 깨울 이유가 없다.
- **묶음으로 도착한다**: 한 번의 응답에 0건·1건·여러 건. 그래서 갱신 경계에
  `— 16:42 갱신 · 3건 —` 구분선을 남기고, 줄은 120ms 간격으로 쌓아 읽히게 한다.
- **0건인 갱신이 정상이다**. 참가자가 적으면 조용한 1분은 흔하다 — 그땐 "새 기록 없음" 과
  마지막 기록이 몇 분 전인지만 바뀌고 live 점이 회색으로 죽는다.
- 첫 로드에서 이벤트가 하나도 없으면 **박스 자체를 숨긴다**(빈 티커는 조용한 게 아니라 죽은 걸로 읽힌다).
- **스냅샷 주기를 60초로 당기지 않는다** — KV 쓰기가 하루 1,440회가 되어 무료 한도(1,000/일)를 넘는다.
  지금 5분(288회/일)이 그 한도에 맞춘 값이다(§13). 가벼운 `/activity` 가 유일하게 싼 길이다.

---

## 26. shipping 파비콘 — `GET /icon/<host>.png` (2026-08-26)

보드의 main shipping 옆에 그 서비스의 파비콘을 띄운다. 아이콘 하나로 줄이 "제품 목록"처럼 읽힌다.

- **제3자 파비콘 API 를 쓰지 않는다.** `google.com/s2/favicons?domain=…` 는 한 줄이면 되지만
  방문자 브라우저가 제3자를 직접 때린다 — 누가 이 보드를 보고 있는지가 그쪽에 남는다.
  "내용은 수집하지 않는다"를 1번 규칙으로 파는 서비스가 할 일이 아니다.
- **우리가 한 번 받아 KV 에 굽는다**(`icon:v1:<host>`, TTL 30일). 실패는 하루만 기억(TTL 1일) —
  상대가 고치면 다음 날 다시 시도한다.
- **`/favicon.ico` 한 방으로는 안 된다**(실측): 등록된 두 프로젝트 모두 404 였다 —
  opencodewar.dev 는 `/favicon.svg`, gaemilab.com 은 `/icon.png`. 그래서 HTML 의
  `<link rel="icon">` 을 HTMLRewriter 로 읽고(엣지 스트리밍이라 싸다), 없으면 `/favicon.ico` 폴백.
- **남용 방지**: 요청 호스트가 실제로 누군가의 `users.projects` 에 등록된 도메인일 때만 받아온다
  (KV 미스일 때만 검사). 아니면 404 — 임의 이미지 프록시로 쓰이지 않게.
- 한도: 원본 ≤100KB · 타임아웃 3초 · 이미지 content-type 만 · 후보 4개까지.
- **폴백은 모노그램**: 404 면 웹이 첫 글자 타일(이름 해시 색)로 바꿔 그린다. `<img onerror>` 를
  렌더 후 JS 로 걸어(인라인 핸들러 없이) 줄이 무너지지 않게 한다.
- URL 은 `.png` 로 고정하지만 **응답 content-type 은 원본 그대로**(svg·ico 등). 확장자는 캐시 키
  모양일 뿐 포맷 약속이 아니다 — `<img>` 는 content-type 을 따른다.
