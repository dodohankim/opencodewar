...
- **본인 예외**: 요청의 웹 세션(§14.9)이 프로필 주인과 같으면 비매칭 라벨도 실명으로 내려준다
  (자기 상세는 자기가 본다). 미지정 NULL 은 본인에게도 `""`.
- shipping 에서 프로젝트를 지우면 그 라벨은 즉시 "기타"로 강등된다(과거 이벤트 포함) — 별도 마이그레이션 불필요.

### 20.5 웹 표기

- 상세 차트(30일·하루 뷰)의 스택 기준 토글: **에이전트 ↔ 프로젝트**. 프로젝트 데이터가
  하나도 없으면 토글·범례를 숨긴다(기존 화면과 동일).
- 프로젝트 색: shipping 순서대로 고정 팔레트 5색 + 기타(회색). 범례·툴팁에 라벨 표기.
- 본인 로그인 시 비 shipping 라벨은 흐린 스타일로 구분(공개되지 않는 항목임을 암시).
...
- 명시 링크(`projectDirs`)가 항상 우선. 서버·공개 규칙은 그대로다 —
  자동 라벨은 비 shipping 라벨이므로 공개엔 "기타", 본인 로그인 시에만 실명(§20.4).
